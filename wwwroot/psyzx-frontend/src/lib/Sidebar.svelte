...
<aside class:open={isMobileOpen}>
    ...
    <nav>
        {#each menu as item}
            <a 
                href="{item.id}" 
                class:active={currentHash === item.id || (item.id === '#/' && (currentHash === '' || currentHash === '#/'))} 
                on:click={closeSidebar}
            >
...
<style>
    ...
    a {
        display: flex; align-items: center; gap: 16px; padding: 12px 16px; color: rgba(255,255,255,0.6);
