<script>
  ...
  $: fileExt =
    track && track.filePath
      ? track.filePath.split(".").pop().toUpperCase()
      : "UNK";
  ...
  $: if (track && album) {
    updateMediaSession(track, album, {
      ...
      seekRelative: (offset) => {
        activePlayer.currentTime = Math.max(
          0,
          Math.min(activePlayer.duration, activePlayer.currentTime + offset),
        );
      },
    });
}
  ...
  onMount(() => {
    ...
    if (savedPl && $currentPlaylist.length === 0) {
      try {
        ...
        setTimeout(() => {
          activePlayer.currentTime = parseFloat(savedTime) || 0;
      ...
      if (track) {
        albumsMap.update((map) => {
          ...
          if (targetAlbum) {
            ...
            const targetTrack = targetAlbum.tracks.find(
              (t) => t.id === track.id,
            );
            if (targetTrack)
  ...
  const handleSeek = (e) => {
    ...
    activePlayer.currentTime =
      ((e.clientX - rect.left) / rect.width) * $playerDuration;
  };

  $: nextTrackIndex =
    $isRepeat === "all" && $currentIndex === $currentPlaylist.length - 1
      ? 0
      : $currentIndex + 1;
...
<audio bind:this={audioElA} crossorigin="anonymous" playsinline preload="auto"
></audio>
<audio bind:this={audioElB} crossorigin="anonymous" playsinline preload="auto"
></audio>
...
>
  <div id="progress-wrapper">
    <span id="time-current" class="hide-on-mobile"
      >{formatTime($playerCurrentTime)}</span
    >
    <div
    ...
    <span id="time-total" class="hide-on-mobile"
      >{formatTime($playerDuration)}</span
    >
  </div>
  ...
  <div id="player-main">
    ...
    <div id="controls">
      <button
        class="btn-icon hide-on-mobile"
        class:active={$isShuffle}
        on:click={() => isShuffle.set(!$isShuffle)}
      >
        <svg
          width="20"
          height="20"
          ...
          /><path
            d="m18 2 4 4-4 4M2 6h1.9c1.5 0 2.9.9 3.6 2.2M22 18h-5.9c-1.3 0-2.6-.7-3.3-1.8l-.5-.8M18 14l4 4-4 4"
          /></svg
        >
      </button>
      ...
      <button
        class="btn-icon hide-on-mobile"
        class:active={$isRepeat !== "off"}
        on:click={() => {
          const cycle = { off: "all", all: "one", one: "off" };
          isRepeat.set(cycle[$isRepeat] || "off");
        }}
      ...
      >
        ...
        {#if $isRepeat === "one"}
          <span
            style="position: absolute; top: -2px; right: -4px; font-size: 9px; font-weight: 900; color: var(--accent-color); background: var(--bg-primary, #111); border-radius: 50%; width: 14px; height: 14px; display: flex; align-items: center; justify-content: center; line-height: 1;"
            >1</span
          >
        {/if}
      </button>
    ...
    <div id="nerdy-info" class="hide-on-mobile">
        ...
        <div class="kbps-badge">
          <svg
            ...
            ><rect x="3" y="8" width="4" height="8" /><rect
              ...
              width="4"
              height="16"
            /><rect x="17" y="10" width="4" height="4" /></svg
          >
          <span>{bitrate} kbps</span><span class="ext">{fileExt}</span>
        </div>
      {:else}
        <span style="font-size: 11px; font-family: monospace; color: #555;"
          >NO SIGNAL</span
        >
      {/if}
      ...
      {#if $connectedDevices && $connectedDevices.length > 0}
        <button
          class="kbps-badge sync-btn"
          class:active-sync={$targetDeviceId && $targetDeviceId !== $myDeviceId}
      ...
      <ul class="device-list">
        ...
        {#each $connectedDevices as dev}
          <li
            ...
            on:click={() => {
              selectTargetDevice(dev.deviceId);
              showSyncModal = false;
            }}
          >
            <svg
              ...
              stroke="currentColor"
              stroke-width="2"
              ><rect x="4" y="2" width="16" height="20" rx="2" ry="2" /><line
                x1="12"
                y1="18"
                x2="12.01"
                y2="18"
...
<style>
  ...
  .volume-slider::-webkit-slider-thumb {
    ...
    height: 16px;
    border-radius: 50%;
    background: radial-gradient(circle at center, #000 24%, transparent 25%),
      radial-gradient(circle at center, #333 26%, transparent 32%),
      var(--metal-shine), linear-gradient(135deg, #fff 0%, #666 100%);
  ...
  @media (min-width: 769px) {
    :global(footer#player.layout-swapped #player-main) {
    ...
    :global(footer#player.layout-swapped #now-playing) {
      align-items: flex-end;
      margin-left: auto;
      margin-right: 14px;
    }
    :global(footer#player.layout-swapped #nerdy-info) {
